...
export const sendDisabledUserEmail = async (userId) => {
  try {
    const res = await axios.post(
      `${API}/api/v1/admin/users/${userId}/send-disabled-email`,
      {},
      getAuthHeaders()
    );
    return res.data;
  } catch (error) {
    console.error(`Error sending disabled email to user ${userId}:`, error);
    throw error;
  }
};
